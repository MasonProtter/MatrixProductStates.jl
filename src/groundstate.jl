...
function sweep!(::Right, ψ::MPS{L, T}, H::MPO{L, T}, R_exs, hs) where {L, T}
    L_exs = Array{T, 3}[]
    L_ex  = ones(T, 1, 1, 1)
    E = zero(T)
    for l in 1:(L-1)
        ...
        E, A, SVp = eigenproblem(right, ψ[l], L_ex, W, R_exs[l], hs[l])
        ψ.tensors[l] = A
...
function sweep!(::Left, ψ::MPS{L, T}, H::MPO{L, T}, L_exs, hs) where {L, T}
    R_exs = Array{T, 3}[]
    R_ex  = ones(T, 1, 1, 1)
    E = zero(T)
    for l in L:-1:2
        ...
        E, US, B = eigenproblem(left, ψ[l], L_exs[l-1], W, R_ex, hs[l])
        ψ.tensors[l] = B
...
function h_matrix!(L_ex::Array{T,3}, W::Array{T,4}, R_ex::Array{T,3}, h) where {T}
    @tensor h[σˡ, aˡ⁻¹, aˡ, σˡ′, aˡ⁻¹′, aˡ′] = L_ex[bˡ⁻¹, aˡ⁻¹, aˡ⁻¹′] * W[bˡ⁻¹, bˡ, σˡ, σˡ′] * R_ex[bˡ, aˡ, aˡ′]
    @cast h[(σˡ, aˡ⁻¹, aˡ), (σˡ′, aˡ⁻¹′, aˡ′)] := h[σˡ, aˡ⁻¹, aˡ, σˡ′, aˡ⁻¹′, aˡ′]
end

function eigenproblem(dir::Direction, M::Array{T, 3}, L_ex::Array{T, 3}, W::Array{T, 4}, R_ex::Array{T, 3}, h) where {T}
    @cast v[(σˡ, aˡ⁻¹, aˡ)] |= M[aˡ⁻¹, aˡ, σˡ]
    
    h = h_matrix!(L_ex, W, R_ex, h)
    λ, Φ, _, niter = eigs(h, v0=v, nev=1, which=:SR)
    @show niter
    E  = λ[1]::T 
    v⁰ = (Φ[:,1])::Vector{T}
...
function ground_state(ψ::MPS{L, T}, H::MPO{L, T}; maxiter=10, quiet=false, ϵ=1e-8) where {L, T}
    ϕ = ψ |> copy
    quiet || println("Preallocating arrays")
    ...
    while not(converged)
        ...
        L_exs, E₀′ = sweep!(right, ϕ, H, R_exs, hs)

        quiet || println("Performing left sweep")
        R_exs, E₀  = sweep!(left,  ϕ, H, L_exs, hs)
