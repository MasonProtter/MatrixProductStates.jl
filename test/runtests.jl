...
@testset "Hubbard" begin
    ...
    const c_dn = id ⊗ c
    const id²  = id ⊗ id
    const n_up = c_up' * c_up
    const n_dn = c_dn' * c_dn
    const Û = U*(n_up * n_dn) - μ*(n_up + n_dn)
    const P_up = (id² - 2c_up'*c_up) # Spin up parity operator
    const P_dn = (id² - 2c_dn'*c_dn) # Spin down parity operator
