...
using MatrixProductStates, LinearAlgebra
...
Initialize a random, left canonical state
#+BEGIN_SRC jupyter-julia
ψ = randn(MPS{L, Float64}, 1000, 2) |> leftcanonical |> rightcanonical
#+END_SRC

#+RESULTS:
: Matrix product state on 20 sites
:   Physical dimension: 2
:   Bond dimensions:   (1, 2) × (2, 4) × (4, 8) × (8, 16) × (16, 32) × (32, 64) × (64, 128) × (128, 256) ×  ... × (2, 1)
...
Compression: 
#+BEGIN_SRC jupyter-julia
ψ̃ = compress(ψ, right, Dcut=700)[1]
#+END_SRC
...
Over-compression: 
#+BEGIN_SRC jupyter-julia
ψ̃ = compress(ψ, right, Dcut=100)[1]
ψ̃' * (H * ψ̃)
#+END_SRC
