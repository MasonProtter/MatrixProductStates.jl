...
function H_TFIM(g, L)
    ...
    σᶻ = [1  0; 
          0 -1]
    H_tnsr = zeros(Complex{Float64}, 3, 3, 2, 2)
    H_tnsr[1, 1, :, :] = id    
    H_tnsr[2, 1, :, :] = -σᶻ  
    H_tnsr[3, 1, :, :] = -g*σˣ
    H_tnsr[3, 2, :, :] = σᶻ   
...
#+END_SRC
Note: on my machine, some "errors" are shown when I type `using
MatrixProductStates` related to `Strided.jl`. These can be ignored,
but I hope to fix them soon.
